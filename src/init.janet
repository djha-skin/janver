      ...
      (if (not= epoch-cmp 0)
        ...
        (let [[just-a just-b] (justify a-parts b-parts)
          ...
          (label compare-parts
                 (var i 0)
                 (while (< i la)
                   (let [first-cmp (non-numbers-compare (get just-a i) (get just-b i))]
                     (when (not= first-cmp 0)
                       (return compare-parts first-cmp)))
                   (++ i)
                   (let [second-cmp (numbers-compare (get just-a i) (get just-b i))]
                     (when (not= second-cmp 0)
                       (return compare-parts second-cmp)))
                   # Due to the nature of the PEG, I can guarantee that there are an
                   # even number of matches for both A and B, so I'm allowed to do
                   # this.
                   (++ i))
                 0))))))
